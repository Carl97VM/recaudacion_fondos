<template>
  <q-page>
    <h6 align="center">Lista de Aportantes</h6>
...
export default defineComponent({
  ...
  setup() {
    ...
    async function cargarAportantes() {
      try {
        let url = "/aportantes/list";
        const res = await api.get(url);
        console.log(res.data);
        rows.value = res.data.data.map((datos) => ({
          names: datos.label,
          ocupacion: datos.profesion_oficio,
    ...
    function verInformesAportante(id) {
      router.push({ name: 'informes-aportante', params: { id } });
    }

    async function eliminarAportante(id) {
      try {
        const url = `/aportantes/${id}/destroy`;
